...
class SequentialTransform(Transform):
    ...
    def forward(self, x):
        total_ld = torch.zeros(x.shape[0], device=x.device, dtype=x.dtype)
...
class InvertibleDenseBlock(Transform):
    ...
    def __init__(self, in_channels, num_steps, num_layers, layer_mid_chnls, growth_rate=None, checkpointing=False):
        ...
        # Don't store these in the state dict
        self.register_buffer('noise_dist_loc', torch.tensor(0.), persistent=False)
        self.register_buffer('noise_dist_scale', torch.tensor(1.), persistent=False)
        self.noise_dist = torch.distributions.Normal(self.noise_dist_loc, self.noise_dist_scale)
    ...
    def cross_unit_coupling(self, x, g):
        # Cross-unit coupling
        N, _, H, W = x.shape
        eta = self.noise_dist.sample(torch.Size([N, self.growth_rate, H, W]))
        mu, log_scale = g(x)
    ...
    def forward(self, x):
        total_ld = torch.zeros(x.shape[0], device=x.device, dtype=x.dtype)
        if self.noise_dist.loc.device != self.noise_dist_loc.device:
            # Update distribution parameter devices
            # My solution to https://github.com/pytorch/pytorch/issues/31009
            self.noise_dist.loc = self.noise_dist_loc
            self.noise_dist.scale = self.noise_dist_scale
        N, _, H, W = x.shape
        g_inputs = []
        x_in = x
